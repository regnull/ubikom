...
func main() {
	...
	instance, err := keyregistry.NewKeyregistry(contractAdr, client)
	if err != nil {
		log.Fatal(err)
	}
	r, err := instance.Registry(nil, privateKey.PublicKey().SerializeCompressed())
	if err != nil {
		log.Fatal(err)
	}
	fmt.Printf("%+v\n", r)
	...
	// instance.Registry(auth, privateKey.PublicKey().SerializeCompressed())
